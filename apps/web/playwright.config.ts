...
export default defineConfig({
  ...
  webServer: [
    {
      command: `rtk uv run uvicorn apps.api.tests.e2e_mcp_server:app --host 127.0.0.1 --port ${mcpPort}`,
      cwd: repoRoot,
    ...
    {
      command: `rtk env DEEP_RESEARCHER_API_TARGET=http://127.0.0.1:${apiPort} npm --prefix apps/web run dev -- --host 127.0.0.1 --port ${webPort}`,
      cwd: repoRoot,
